...
To achieve these savings, we need to know when to stop screening: and existing approaches to deciding this are insufficient [@Shemilt2014; @Howard2020; @Jonnalagadda2013]. We need to to confident that we don't miss studies, and we need to be able to communicate this confidence. 



## Screening documents 

We can set up a toy dataset with 1,000 documents ($N_{tot}$), of which 100 are relevant ($\rho_{tot}$)
...
If we screen these at random, we will see 95% recall after seeing ~95% of all relevant documents
...
It's also easy to tell when 95% recall has been achieved when we know how many relevant documents there are
...
```{r echo=F}
par(pty="s")
r_al <- cumsum(ordered_docs[1:cutoff])[cutoff]
N_al <- cutoff
K <- r_tot-r_al 
N <- N_tot - N_al
```
...
```{r}
plot(c(rep(NA,N_al),p), xlab="Documents screened")
abline(h=.05)
abline(v=N_al+which(p<0.05)[1])
```
...
This presentation: \url{https://github.com/mcallaghan/rapid-screening/pres/rapid-review-esmar.Rmd}
