...
%%
	...
	We start by explaining the sampling and heuristic based stopping criteria before showing with toy data how the criteria falls short. 
...
\subsection*{A Statistical Stopping Criterion for Active Learning}
	...
	We evaluate each of the three criteria discussed on real world test data. With each method, we run 100 runs and record the recall error and actual work saved each time. We operationalise the heuristic stopping criteria with 10, 20, and 50 irrelevant records in a row. For simplicity, we use a basic SVM model [REFS], with 1-2 word n-grams in the document abstracts used as input data. We start with random samples of [100, 200, and 500] documents.
